...
export class VoiceController {
  ...
  constructor(opts = {}) {
    ...
    // New mobile policy (defaults keep current behavior)
    this.mobilePolicy = opts.mobilePolicy || 'auto';
    // 'auto'            → block Web Speech on installed mobile PWA if mic is active
    // 'force-webspeech' → always try Web Speech (not recommended on Android PWA)
    // 'push-to-talk'    → pause mic, run SR, then resume (hooked via callbacks below)

    // Optional hooks used only when mobilePolicy === 'push-to-talk'
    this.onRequestPauseMic = typeof opts.onRequestPauseMic === 'function' ? opts.onRequestPauseMic : null;
    this.onRequestResumeMic = typeof opts.onRequestResumeMic === 'function' ? opts.onRequestResumeMic : null;

    // Track whether WebRTC mic is currently live; device.js will set this.
  ...
  /** Tell voice.js whether the WebRTC mic is currently live */
  setMicActive(v) { this._micActive = !!v; }

  /** Optional: begin push-to-talk on mobile PWA */
  async beginPushToTalk() {
    if (this.mobilePolicy !== 'push-to-talk') return this.start();
    if (isMobile() && isInstalledPWA() && this._micActive && this.onRequestPauseMic) {
      try { await this.onRequestPauseMic(); } catch { }
    }
    const ok = this.start();
    if (!ok && this.onRequestResumeMic) {
      try { await this.onRequestResumeMic(); } catch { }
    }
    return ok;
  }
  ...
  start() {
    ...
    // Avoid the Android installed-PWA conflict (Web Speech + active WebRTC mic)
    if (this.mobilePolicy !== 'force-webspeech' && isMobile() && isInstalledPWA()) {
      if (this._micActive) {
        if (this.mobilePolicy === 'push-to-talk' && this.onRequestPauseMic) {
          // Let caller handle push-to-talk (pause mic first, then call beginPushToTalk)
          this.onError('mobile_pwa_mic_conflict');
          return false;
        }
        // Default: fail fast with a clear, actionable error
        this.onError('mobile_pwa_mic_conflict');
        return false;
      }
    }
  ...
  _onError(ev) {
    ...
    // Avoid restart loop in the known mobile PWA + mic conflict
    if (isMobile() && isInstalledPWA() && this._micActive && this.mobilePolicy !== 'force-webspeech') {
      this._listening = false;
      this.onListenStateChange(false);
      return;
    }
  ...
  _onEnd() {
    // Avoid restart loop on mobile PWA + mic conflict
    if (isMobile() && isInstalledPWA() && this._micActive && this.mobilePolicy !== 'force-webspeech') {
      this._listening = false;
      this.onListenStateChange(false);
      return;
