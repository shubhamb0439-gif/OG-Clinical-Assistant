(function () {
  ...
  // ── ORB BUTTON: delegates directly to the hidden btnVoice click handler ──
  // This means ui.js's existing elBtnVoice listener fires — no new logic,
  // no new SpeechRecognition instance, no duplicate start() calls.
  if (orbBtn && hiddenVoice) {
    orbBtn.addEventListener('click', function () {
      hiddenVoice.click();
    });
  }

  // Keep orb visual in sync with the actual isListening state by watching
  // the hidden button's text (ui.js sets it to 'Stop Voice' when active).
  setInterval(function () {
    if (!orbBtn || !hiddenVoice) return;
    var txt = hiddenVoice.textContent.trim().toLowerCase();
    var isActive = txt.indexOf('stop') >= 0;
    if (isActive) {
  ...
  // ── STREAM ORB: same delegation, no duplicate listeners ──
  if (streamOrbBtn && hiddenVoice) {
  ...
  // ── MSG ORB: same delegation ──
  if (msgOrbBtn && hiddenVoice) {
  ...
  function syncStreamControlLabels() {
    if (streamMuteBtn && hiddenMute) {
      ...
      streamMuteBtn.textContent = muteText.toLowerCase().indexOf('unmute') >= 0 ? 'Unmute' : 'Mute';
      if (muteText.toLowerCase().indexOf('unmute') >= 0) {
    ...
    if (streamHideBtn && hiddenVideo) {
      ...
      streamHideBtn.textContent = videoText.toLowerCase().indexOf('show') >= 0 ? 'Show' : 'Hide';
      if (videoText.toLowerCase().indexOf('show') >= 0) {
