...
(function () {
  ...
  // SIGNAL URL priority: ?signal → injected → stored → XR_CONFIG.HUB_URL → same-origin → ngrok fallback
  ...
  const hubFromConfig = (typeof window !== 'undefined' && window.XR_CONFIG && window.XR_CONFIG.HUB_URL) || null;
  ...
  window.SIGNAL_URL = pick(override, injected, stored, hubFromConfig, sameOrigin, ngrokFallback);
  // Back-compat alias (no behavior change)
  window.HUB_URL = window.SIGNAL_URL;
