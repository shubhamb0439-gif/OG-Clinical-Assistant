...
io.on('connection', (socket) => {
  ...
  // -------- join (duplicate-guarded) --------
  socket.on('join', async (xrId) => {
    dlog('[EVENT] join', xrId);

    // 🚫 Hard duplicate block: if another socket already owns this XR-ID, reject this one
    const existing = clients.get(xrId);
    if (existing && existing.id !== socket.id) {
      dlog('[JOIN] Duplicate XR-ID detected, rejecting:', { xrId, holderSocket: existing.id, newSocket: socket.id });
      try {
        socket.emit('duplicate_id', {
          xrId,
          holderInfo: { deviceName: existing.data?.deviceName || 'Unknown', socketId: existing.id }
        });
      } catch (_) { }
    ...
    // ✅ Register
    socket.data.xrId = xrId;
    try { await socket.join(roomOf(xrId)); } catch (e) { dwarn('[JOIN] join room failed:', e?.message || e); }
    clients.set(xrId, socket);
    onlineDevices.set(xrId, socket);
  ...
  // -------- identify (duplicate-guarded, no replacement) --------
